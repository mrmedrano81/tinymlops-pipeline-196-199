...
def create_model(model_settings, model_architecture, model_size_info, is_training):
    ...
    #added micronets model
    elif model_architecture == 'micronets':
        return create_micronets_model(model_settings, model_size_info)
...
#added micronets model as interpreted from source paper
def create_micronets_model(model_settings, model_size_info):
    """Builds a micronets model with convolutional & depthwise separable convolutional layers.

    For more details see https://arxiv.org/pdf/2010.11267.pdf
    ...
    Changes from create_ds_cnn_model:
        -RelU() -> ReLU(max_value=6)

    Returns:
        tf.keras Model of the Micronets model based on the 'DS-CNN' architecture.
    """
