...
def train():
    ...
    # added cosine decay (mike)
    if FLAGS.optimizer_function == 'cosine_decay':
        print("\nusing cosine decay optimizer\n")
        cosine_decay_scheduler = tf.keras.optimizers.schedules.CosineDecay(
            learning_rates_list[0], training_steps_list[0], FLAGS.cosine_decay_alpha, name=None
        )
        optimizer = tf.keras.optimizers.Adam(learning_rate=cosine_decay_scheduler)

    else:
        # default from original repo (mike)
        print("\nusing piecewise constant decay optimizer\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default='adam',
        help='what optimizer function to use for learning rate decay')
    parser.add_argument(
        '--cosine_decay_alpha',
        type=float,
        default='0.004',
        help='alpha value for cosine decay optimizer (desired final lr divided by initial lr)')
    #----------------------------------------------------#
